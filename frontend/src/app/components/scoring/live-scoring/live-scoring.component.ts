...
export class LiveScoringComponent implements OnInit, OnDestroy {
  ...
  viewMatchDetails(match: LiveMatchSummary): void {
    // Navigate to detailed match view or open in dialog
  ...
  exportMatchData(match: LiveMatchSummary): void {
    // Implementation for exporting match data
    this.snackBar.open('Exporting match data...', 'Close', { duration: 2000 });
  }
